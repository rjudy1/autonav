...
class MainRobot(AutoNavNode):
    ...
    def line_following_state(self):
        ...
        if self.waypoint_found:  # reached gps waypoint - switch to gps navigation
            self.waypoint_found = False
            self.state_pub.publish(self.GPS_NAVIGATION)
            self.state = State.GPS_Navigation
            self.wheel_pub.publish(WHEELS_GPS_NAV)
            ...
            self.obj_seen = False
            self.state_pub.publish(self.LINE_TO_OBJECT)
            self.state = State.Line_To_Object
            self.wheel_pub.publish(WHEELS_TRANSITION)
    ...
    def object_avoidance_from_line_state(self):
        ...
        if self.obj_seen:
            ...
            self.found_line = False
            self.state_pub.publish(self.OBJECT_TO_LINE)
    ...
    def object_avoidance_from_gps_state(self):
        ...
        if self.obj_seen:
            self.obj_seen = False
            self.state_pub.publish(self.GPS_TO_OBJECT)
            ...
            self.waypoint_straight = False
            self.state_pub.publish(self.GPS_NAVIGATION)
            selt.state = State.GPS_Navigation
            self.wheel_pub.publish(WHEELS_GPS_NAV)
    ...
    def gps_navigation_state(self):
        ...
        # After looking for an obstacle, see if we have arrived
            ...
            self.waypoint_found = False
            self.state_pub.publish(self.FIND_LINE)
    ...
    def line_to_object_state(self):
        ...
        self.wheel_pub.publish(TRANSITION_CODE + str(25) + "," + str(0))
        if self.path_clear:
            self.path_clear = False
            self.state_pub.publish(self.OBJECT_AVOIDANCE_FROM_LINE)
            self.state = State.Object_Avoidance_From_Line
            self.wheel_pub.publish(WHEELS_OBJECT_AVOIDANCE)
    ...
    def object_to_line_state(self):
        ...
        if self.aligned:
            self.aligned = False
            self.state_pub.publish(self.LINE_FOLLOWING)
            self.state = State.Line_Following
            self.wheel_pub.publish(WHEELS_LINE_FOLLOWING)
    ...
    def gps_to_object_state(self):
        ...
        self.lights_pub.publish("G_Off")
        # Just keep turning until the object is not in front of us
        self.wheel_pub.publish(TRANSITION_CODE + str(0) + "," + str(0))
        if self.path_clear:
            self.path_clear = False
            self.state_pub.publish(self.OBJECT_AVOIDANCE_FROM_GPS)
    ...
    def find_line_state(self):
        ...
        if self.found_line:
            self.found_line = False
            self.state_pub.publish(self.LINE_ORIENT)
    ...
    def gps_callback(self, gps_event):
        ...
        try:
            if gps_event == WAYPOINT_FOUND:
                self.waypoint_found = True
            elif gps_event == WAYPOINT_STRAIGHT:
                self.waypoint_straight = True
            else:
                rospy.logwarn("Unknown Message")
                rospy.logwarn(line_event.data)
        finally:
            # Release the lock
            self.lock.release()
